...
const companyid = JSON.parse(localStorage.getItem("loginInfo")).CompanyId;
var storeid = 0;
$(function () {
	"use strict";
	...
	function cb(start, end) {
		...
		from = start.format('MMMM D, YYYY');
		to = end.format('MMMM D, YYYY');
		storeid = +$('#store_select')[0].value;
		var settings = {
			"async": true,
			"crossDomain": true,
			"url": `https://biz1pos.azurewebsites.net/api/Dashboard/Post/?fromDate=${start.format('YYYY-MM-DD')}&toDate=${end.format('YYYY-MM-DD')}&compId=${companyid}&storeId=${storeid}`,
			"method": "GET",
			"headers": {
		...
		$.ajax(settings).done(function (response) {
			try {
				...
				if (response.hasOwnProperty('TotalSales')) {
					...
					$('#payments')[0].innerText = (TotalSales == null) ? 0 : nfObject.format(TotalSales);
